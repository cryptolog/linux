...
static noinline void __init grouper_setup_bluesleep(void)
{
	...
	grouper_bluesleep_resources[2].start =
		grouper_bluesleep_resources[2].end =
...
static int elan_touch_init(void)
{
	...
	printk("[ELAN] Touch driver register\n");
...
struct platform_device *tegra_usb_hsic_host_register(struct platform_device *ehci_dev)
{
	...
	pdev = platform_device_alloc(ehci_dev->name,
		ehci_dev->id);
	if (!pdev)
		return NULL;
